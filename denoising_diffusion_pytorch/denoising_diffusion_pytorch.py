...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, mask, text_cond_top, text_cond_bot, t, clip_denoised: bool):
        
        x_recon = self.predict_start_from_noise(x, t=t, noise=self.denoise_fn(x, mask, text_cond_top, text_cond_bot, t))
    ...
    @torch.no_grad()
    def p_sample(self, x, mask, text_cond_top, text_cond_bot, t, clip_denoised=True, repeat_noise=False):
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance = self.p_mean_variance(x=x, mask=mask, text_cond_top=text_cond_top, text_cond_bot=text_cond_bot, t=t, clip_denoised=clip_denoised)
    ...
    @torch.no_grad()
    def p_sample_loop(self, x, mask, text_cond_top, text_cond_bot):
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            img = self.p_sample(img, mask, text_cond_top, text_cond_bot, torch.full((b,), i, device=device, dtype=torch.long))
            img[mask.bool()] = x[mask.bool()].clone()
    ...
    @torch.no_grad()
    def conditional_sample(self, batch_size, conditions, text_cond_top, text_cond_bot):
        '''
        ...
        return self.p_sample_loop(x, mask, text_cond_top, text_cond_bot)
    ...
    def p_losses(self, x_start, mask, text_cond_top, text_cond_bot, t, noise = None):
        b, h, w = x_start.shape
        noise = default(noise, lambda: conditional_noise(mask, x_start))

        x_noisy = self.q_sample(x_start=x_start, mask=mask, t=t, noise=noise)
        # x_noisy[mask.bool()] = x_start[mask.bool()].clone()
        x_recon = self.denoise_fn(x_noisy, mask, text_cond_top, text_cond_bot, t)
    ...
    def forward(self, x, mask, text_cond_top, text_cond_bot, *args, **kwargs):
        # import pdb
        # pdb.set_trace()
        # print(x.size())
        ...
        return self.p_losses(x, mask, text_cond_top, text_cond_bot, t, *args, **kwargs)
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            for i in range(self.gradient_accumulate_every):
                data, mask, text_cond_top, text_cond_bot = next(self.dl)
                data = data.cuda()
                mask = mask.cuda()
                text_cond_top = text_cond_top.cuda()
                text_cond_bot = text_cond_bot.cuda()
                loss = self.model(data, mask, text_cond_top, text_cond_bot)
            ...
            if self.step % self.save_and_sample_every == 0:
                ...
                for i, conditions in enumerate(conditions_l):
                    ## [ -1, 1 ]
                    all_images = self.ema_model.conditional_sample(batch_size=1, conditions=conditions, text_cond_top=text_cond_top.detach()[0:1], text_cond_bot=text_cond_bot.detach()[0:1])
                    ## [ 0, 1 ]
                    all_images = (all_images + 1) * 0.5
                    ## unnormalize
                    unnormed = self.ds.unnormalize(all_images)
