...
class TemporalUnet(nn.Module):
    ...
    def __init__(
        ...
        self.text_dim = 512
        self.text_cond_top_mlp = nn.Sequential(
            #SinusoidalPosEmb(cond_dim),
            nn.Linear(self.text_dim, dim * 2),
            nn.Mish(),
            nn.Linear(dim * 2, dim),
        )
        self.text_cond_bot_mlp = nn.Sequential(
            #SinusoidalPosEmb(cond_dim),
            nn.Linear(self.text_dim, dim * 2),
            nn.Mish(),
            nn.Linear(dim * 2, dim),
    ...
    def forward(self, x, cond, text_cond_top, text_cond_bot, time):
        '''
        ...
        top_c = self.text_cond_top_mlp(text_cond_top.float().cuda())
        bot_c = self.text_cond_bot_mlp(text_cond_bot.float().cuda())
        c = torch.cat([top_c, bot_c], dim=-1)
