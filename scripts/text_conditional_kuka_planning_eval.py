...
from denoising_diffusion_pytorch.datasets.tamp import KukaDataset, KukaDatasetReward
...
def eval_episode(model, env, dataset, idx=0):
    ...
    clip_model.float()
    clip_model.eval()
    text_cond = clip_model.encode_text(clip.tokenize(["red", "green", "blue", "yellow", ""]).to("cuda"))
    k = 70
    #text_cond_top = torch.stack([text_cond[1] for _ in range(40)]+[text_cond[2] for _ in range(40)]+[text_cond[3] for _ in range(48)]).unsqueeze(0)
    #text_cond_bot = torch.stack([text_cond[0] for _ in range(40)]+[text_cond[1] for _ in range(40)]+[text_cond[2] for _ in range(48)]).unsqueeze(0)
    ind_text_list = ["red", "green", "blue", "yellow"]
    goal = np.arange(4)
    np.random.shuffle(goal)
    print(ind_text_list[goal[0]], ind_text_list[goal[1]], ind_text_list[goal[2]], ind_text_list[goal[3]])
    for i in range(3):
        ...
        text_cond_top = torch.stack([text_cond[goal[i+1]] for _ in range(k)]+[text_cond[-1] for _ in range(128-k)]).unsqueeze(0)
        text_cond_bot = torch.stack([text_cond[goal[i]] for _ in range(k)]+[text_cond[-1] for _ in range(128-k)]).unsqueeze(0)
        samples = samples_orig = trainer.ema_model.conditional_sample(1, conditions, text_cond_top=text_cond_top.detach(), text_cond_bot=text_cond_bot.detach())
...
#### dataset
# H = 128
# dataset = KukaDatasetReward(H)

env_name = "multiple_cube_kuka_convnew_real2_128_text_cond"
H = 128
T = 1000
